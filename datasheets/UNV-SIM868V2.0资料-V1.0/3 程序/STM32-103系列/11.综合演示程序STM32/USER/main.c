...
void TIM2_IRQHandler(void) //TIM3中断
{
	...
	if (TIM_GetITStatus(TIM2, TIM_IT_Update) != RESET) //检查TIM3更新中断发生与否
	{
		...
		if (Time_miao >= 1)
			Time_miao++; //Time_miao==1时启动计数，溢出自动停止
		if (Timer0_start)
			Times++;
		if (Times > (shijian))
		{
		...
		if (sim_ready)
		{
			if (USART_RX_STA & 0x8000)
			{
				...
				if (strstr((char *)Uart1_Buf, "拨号"))
					GSM_call_test(); //电话测试
				else if (strstr((char *)Uart1_Buf, "短信"))
					GSM_sms_test(); //短信测试
				else if (strstr((char *)Uart1_Buf, "GPRS"))
					GSM_gprs_test(); //GPRS测试
				else if (strstr((char *)Uart1_Buf, "定位"))
					GSM_jz_test(); //基站测试
...
u8 GSM_call_test()
{
	...
	while (1)
	{
		if (USART_RX_STA & 0x8000)
		{
			...
			USART_RX_STA = 0;
			if (strstr((const char *)Uart1_Buf, "退出"))
				return 0;
			if (strstr((const char *)Uart1_Buf, "接听"))
				mode = 1;
			else if (strstr((const char *)Uart1_Buf, "挂断"))
				mode = 2;
			else if (strstr((const char *)Uart1_Buf, "致电"))
				mode = 3;
			else if (mode == 0)
				mode = 0; //无效
			else
...
u8 GSM_sms_test()
{
	...
	while (1)
	{
		if (USART_RX_STA & 0x8000)
		{
			...
			if (strstr((const char *)Uart1_Buf, "退出"))
				return 0;
			if (strstr((const char *)Uart1_Buf, "设置"))
				mode = 3; //设置短信中心号
			else if (strstr((const char *)Uart1_Buf, "查看"))
				mode = 2; //查看短信
...
u8 GSM_gprs_test()
{
	...
	while (1)
	{
		if (USART_RX_STA & 0x8000)
		{
			...
			if (strstr((const char *)Uart1_Buf, "退出"))
				mode = 4; //退出
			else if (strstr((const char *)Uart1_Buf, "设置"))
				mode = 1; //设置配置
			else if (strstr((const char *)Uart1_Buf, "发送"))
				mode = 2; //发送内容
			else
				mode = 0;
		}
		else
		{
			...
			else if ((flag == 1) && ((strstr((const char *)Uart2_Buf, "CLOSED")) && (Time_miao > 10))) //连接失败或超时
			{
